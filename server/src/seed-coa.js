const { PrismaClient, AccountType, PostingType } = require('@prisma/client');
...
async function createGroupsRecursively(companyId, groups, parentId = null) {
    for (const group of groups) {
        ...
        if (group.ledgers && group.ledgers.length > 0) {
            for (const ledger of group.ledgers) {
                ...
                console.log(`    Created ledger: ${ledger.name} (${ledger.code})`);
            }
        }
...
async function seedChartOfAccounts() {
    try {
        ...
        for (const company of companies) {
            ...
            if (existingGroups > 0) {
                console.log(`✓ ${company.businessName} already has ${existingGroups} ledger groups`);
                continue;
            }

            console.log(`\nSeeding COA for ${company.businessName}...`);
            await createGroupsRecursively(company.id, DEFAULT_GROUPS);
            console.log(`✅ Done for ${company.businessName}`);
        }
