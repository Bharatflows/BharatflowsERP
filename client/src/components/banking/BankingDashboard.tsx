...
export function BankingDashboard({ onNavigate, hideStats = false }: BankingDashboardProps) {
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm p-6">
          ...
          <div className="space-y-4">
            {accountBalances.map((account) => (
              <div key={account.id} className="p-4 bg-background/50 rounded-lg border border-border hover:bg-muted/30 transition-all group">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <div className={cn("p-2.5 rounded-full",
                      account.type === 'Credit' ? "bg-amber-50 text-amber-600" : "bg-primary/10 text-primary"
                    )}>
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {account.provider && (
                          <span className="px-1.5 py-0.5 rounded-md bg-blue-50 text-2xs font-bold text-blue-600 uppercase tracking-tight">
                            {account.provider}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="text-right">
                    ...
                    {account.lastSyncAt ? (
                      <p className="text-2xs text-muted-foreground">Synced: {new Date(account.lastSyncAt).toLocaleTimeString()}</p>
                    ) : (
                  ...
                  <div className="mt-3 flex items-center justify-between pt-3 border-t border-border/10">
                    {account.provider ? (
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 text-overline gap-2 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50"
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 text-overline gap-2 text-primary hover:text-primary hover:bg-primary/5"
                    ...
                    <Button variant="ghost" size="sm" className="h-8 text-overline text-muted-foreground hover:text-foreground">
                      Details
                    </Button>
                  </div>
                )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm p-6">
          ...
          <div className="space-y-2">
            <Button className="w-full justify-start hover:bg-muted" variant="outline" onClick={() => onNavigate("transactions")}>
              Add Expense / Income
            </Button>
            <Button className="w-full justify-start hover:bg-muted" variant="outline" onClick={() => onNavigate("accounts")}>
              Link New Bank Account
            </Button>
            <Button className="w-full justify-start hover:bg-muted" variant="outline" onClick={() => onNavigate("reconciliation")}>
