...
import { useCreatePurchaseBill, useUpdatePurchaseBill } from "../../hooks/usePurchase";
import { toast } from "sonner";
import {
  Trash2, Save, ChevronLeft, Search, Loader2, PlusCircle, Send
} from "lucide-react";
...
import { INDIAN_STATES, getStateCodeFromGSTIN, isInterStateTransaction } from "../../constants/indianStates";
...
interface BillItem {
  ...
  taxRate: number;
  amount: number;
  trackingData?: { batchNumber?: string; expiryDate?: string; serialNumbers?: string[]; };
}
...
export function CreatePurchaseBill({ billId, onSave, onCancel }: CreatePurchaseBillProps) {
  ...
  const isInterState = isInterStateTransaction(companyStateCode, placeOfSupply);

  useEffect(() => {
    if (selectedSupplier?.gstin) {
      const stateCode = getStateCodeFromGSTIN(selectedSupplier.gstin);
      ...
      try {
        ...
        if (!isEdit && !orderId && !grnId) {
          try {
            ...
            if (response.success && response.data?.nextNumber) setBillNumber(response.data.nextNumber);
          } catch (error) { console.error("Failed to fetch next bill number:", error); }
        }
      } catch (error) {
      ...
      if (orderId) {
        try {
          ...
          if (d) {
            ...
            setNotes(`Bill against PO #${d.orderNumber}`);
            if (d.items && Array.isArray(d.items)) {
              setItems(d.items.map((item: any) => ({
                id: Date.now().toString() + Math.random(), productId: item.productId, productName: item.productName || item.name || "",
                hsnCode: "", quantity: item.quantity || 0, unit: item.unit || "pcs", rate: item.rate || 0, taxRate: item.taxRate || 18, amount: 0,
              })));
            }
          }
        ...
        try {
          ...
          if (d) {
            setSupplierId(typeof d.supplier === 'object' ? d.supplier.id : d.supplierId || "");
            setNotes(`Bill against GRN #${d.grnNumber}`);
            if (d.items && Array.isArray(d.items)) {
              setItems(d.items.map((item: any) => ({
                id: Date.now().toString() + Math.random(), productId: item.productId, productName: item.productName || item.name || "",
                hsnCode: "", quantity: item.receivedQuantity || item.quantity || 0, unit: item.unit || "pcs", rate: 0, taxRate: 18, amount: 0,
              })));
            }
          }
        } catch (error) { toast.error("Failed to load GRN details"); }
      }
    };
    fetchSource();
  }, [orderId, grnId]);
  ...
  useEffect(() => {
    if (isEdit && billId) {
      const fetchBill = async () => {
        try {
          const res = await purchaseService.getBillById(billId);
          const bill = res?.data;
          if (bill) {
            setSupplierId(typeof bill.supplier === 'object' ? bill.supplier.id : bill.supplierId || "");
            ...
            setPaymentTerms(String(bill.paymentTerms || "30"));
            setNotes(bill.notes || "");
            setIsReverseCharge(bill.isReverseCharge ?? false);
            if (bill.placeOfSupply) setPlaceOfSupply(bill.placeOfSupply);
            if (bill.items && Array.isArray(bill.items)) {
              setItems(bill.items.map((item: any) => ({
                id: item.id || Date.now().toString() + Math.random(), productId: item.productId,
                productName: item.productName || (item.product ? item.product.name : ""), hsnCode: item.hsnCode || "",
                quantity: Number(item.quantity) || 0, unit: item.unit || "pcs", rate: Number(item.rate) || 0,
                taxRate: Number(item.taxRate) || 0, amount: 0, trackingData: item.trackingData,
              })));
            }
          }
        } catch (error) { toast.error("Failed to load bill details"); }
      };
      fetchBill();
    }
  }, [isEdit, billId]);
  ...
  const addItem = () => {
    setItems([...items, { id: Date.now().toString(), productId: undefined, productName: "", hsnCode: "", quantity: 1, unit: "pcs", rate: 0, taxRate: 18, amount: 0 }]);
  };
  ...
  const selectProduct = (itemId: string, productId: string) => {
    ...
    setItems(prev => prev.map(item => {
      if (item.id === itemId) {
        return {
          ...item, productId, productName: product.name, hsnCode: product.hsnCode || "", unit: product.unit || "pcs",
          rate: product.purchasePrice || product.sellingPrice || 0, taxRate: product.gstRate || 18
        };
      }
      return item;
    }));
  ...
  const calculateSubtotal = () => {
    if (gstMode === "inclusive") return items.reduce((sum, item) => sum + calculateGST(item.quantity * item.rate, String(item.taxRate), isInterState, "inclusive").baseAmount, 0);
    return items.reduce((sum, item) => sum + (item.quantity * item.rate), 0);
  };
  const calculateTaxBreakdown = () => {
    let totalCGST = 0, totalSGST = 0, totalIGST = 0;
    items.forEach(item => {
      const result = calculateGST(item.quantity * item.rate, String(item.taxRate), isInterState, gstMode);
      totalCGST += result.cgst; totalSGST += result.sgst; totalIGST += result.igst;
    });
    return { totalCGST, totalSGST, totalIGST };
  };
  const calculateTotal = () => {
    if (gstMode === "inclusive") return items.reduce((sum, item) => sum + (item.quantity * item.rate), 0) - discount;
  ...
  const handleSubmit = async (sendAfterSave = false) => {
    ...
    if (!supplierInvoiceNumber) { toast.error("Please enter supplier invoice number"); return; }
    ...
    const billData = {
      ...
      items: items.map(item => ({
        ...
        taxAmount: (() => { const r = calculateGST(item.quantity * item.rate, String(item.taxRate), isInterState, gstMode); return r.cgst + r.sgst + r.igst; })(),
        total: item.quantity * item.rate,
        batchNumber: item.trackingData?.batchNumber,
        expiryDate: item.trackingData?.expiryDate,
        serialNumbers: item.trackingData?.serialNumbers,
      })),
    };

    if (isEdit && billId) {
      updateMutation.mutate({ id: billId, data: billData }, { onSuccess: () => onSave() });
    } else {
      createMutation.mutate(billData, { onSuccess: () => onSave() });
    }
  };
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-card border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-foreground">{isEdit ? "Edit Purchase Bill" : "New Purchase Bill"}</h1>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-card border border-border rounded-xl p-5 space-y-5">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-muted-foreground">Supplier Invoice Number <span className="text-destructive">*</span></Label>
                <Input value={supplierInvoiceNumber} onChange={e => setSupplierInvoiceNumber(e.target.value)} placeholder="e.g. INV-2024-001" className="h-9 text-sm" />
                <p className="text-xs text-muted-foreground">Invoice number from the supplier's bill</p>
              </div>
            </div>
            <div className="bg-card border border-border rounded-xl p-5 space-y-4">
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-muted-foreground">Bill Number</Label>
                <Input value={billNumber} onChange={e => setBillNumber(e.target.value)} placeholder="Auto-generated" className="h-9 font-mono text-sm w-full bg-muted/30 border-border" />
              </div>
              <div className="space-y-1.5">
              ...
              <label className="flex items-center gap-2 text-xs text-muted-foreground cursor-pointer">
                <input type="checkbox" checked={isReverseCharge} onChange={e => setIsReverseCharge(e.target.checked)} className="rounded border-border" />
                Reverse Charge (RCM)
              </label>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-border">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-xs text-muted-foreground">{isInterState ? "IGST" : "CGST+SGST"}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-1.5">
              ...
              <Textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Additional notes..." className="resize-none text-sm min-h-[120px] bg-card border-border" />
            </div>
      ...
      <div className="flex items-center justify-between px-6 py-3.5 bg-card border-t border-border">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => handleSubmit(true)} disabled={isPending} className="h-9 px-5 text-sm font-medium gap-1.5 bg-brand-primary hover:bg-brand-primary-hover text-white shadow-sm">
            {isPending ? <Loader2 className="animate-spin size-4" /> : <Send className="size-4" />} Save & Record
          </Button>
        </div>
      </div>
