...
import { salesService, productsService, partiesService, settingsService, estimatesService } from "../../services/modules.service";
...
export function CreateInvoice({ invoiceId, onSave, onCancel, isDrawer = false }: CreateInvoiceProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (invoiceId) {
          const res = await salesService.getInvoice(invoiceId);
          if (res.success && res.data) {
            // salesService.getInvoice returns { success, data: { invoice } } — extract the actual invoice
            const invoice = (res.data as any)?.invoice || res.data;
            setCustomerId(typeof invoice.customer === 'object' ? invoice.customer.id : invoice.customerId || "");
          ...
          const res = await estimatesService.getById(estimateId);
          if (res.success && res.data) {
            const estimate = res.data;
            setCustomerId(typeof estimate.customer === 'object' ? estimate.customer.id : estimate.customerId || "");
            setNotes(`Converted from Estimate #${estimate.estimateNumber}`);
          ...
          const res = await salesService.getSalesOrder(salesOrderId);
          if (res.success && res.data) {
            const order = res.data;
            setCustomerId(typeof order.customer === 'object' ? order.customer.id : order.customerId || "");
            setNotes(`Converted from Order #${order.orderNumber}`);
          ...
          const res = await settingsService.getNextSequenceNumber('INVOICE');
    ...
    fetchData();
  }, [invoiceId, estimateId, salesOrderId]);
    ...
    setItems(prev => prev.map(item => {
      if (item.id === itemId) {
        ...
        return {
          ...
          rate,
          taxRate: tax,
          amount: (item.quantity * rate) * (1 + tax / 100)
        };
      }
      return item;
    }));
  ...
  const calculateSubtotal = () => {
    if (gstMode === "inclusive") {
      // Back-calculate base amounts from inclusive prices
  ...
  const handleSave = async (sendAfterSave = false) => {
    ...
    try {
      ...
      const res = invoiceId
        ? await salesService.updateInvoice(invoiceId, payload)
        : await salesService.createInvoice(payload);

      if (res.success) {
        toast.success(invoiceId ? "Invoice updated" : sendAfterSave ? "Invoice created & sent" : "Invoice saved as draft");
        onSave(res.data);
      } else {
  ...
  if (initialLoading) {
    return (
      <div className="flex justify-center items-center p-16">
        <Loader2 className="animate-spin size-8 text-brand-primary" />
      </div>
    );
  }
  ...
  return (
    <div className={isDrawer ? "h-full flex flex-col" : "flex flex-col min-h-screen bg-background"}>
      {/* ===== TOP HEADER BAR ===== */}
      <div className="flex items-center justify-between px-6 py-4 bg-card border-b border-border">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={onCancel} className="rounded-full hover:bg-muted size-9">
      ...
      {/* ===== MAIN CONTENT ===== */}
      <div className={`flex-1 overflow-y-auto ${isDrawer ? "px-4 py-4" : "px-6 py-6 max-w-[1100px] mx-auto w-full"}`}>
                ...
                <div className="bg-brand-primary-light border border-brand-primary/20 rounded-lg p-4 flex items-start justify-between">
                  <div>
                    ...
                    {selectedCustomer.gstin && (
                      <p className="text-xs text-muted-foreground mt-0.5">GSTIN: {selectedCustomer.gstin}</p>
                    )}
                    {(selectedCustomer as any).address && (
                      <p className="text-xs text-muted-foreground mt-0.5">{(selectedCustomer as any).address}</p>
                    )}
                  </div>
                  <button
                    type="button"
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
                    ...
                    <tr key={item.id} className="group hover:bg-muted/20 transition-colors">
                      ...
                      <td className="py-3 px-3">
                        <div className="space-y-1">
                          <Select value={item.productId || ""} onValueChange={(val) => {
                            ...
                            <SelectTrigger className="h-8 bg-background border-border hover:border-brand-primary focus:border-brand-primary px-2 text-sm font-medium">
                          ...
                          <Input
                            ...
                            className="h-7 text-xs border-transparent hover:border-border bg-transparent px-1 text-muted-foreground"
                          />
                        </div>
                      </td>
                      <td className="py-3 px-3">
                        <Input
                          ...
                          placeholder="HSN"
                          className="h-8 text-xs font-mono text-center border-transparent hover:border-border bg-transparent"
                        />
                      </td>
                      <td className="py-3 px-3">
                        <Input
                      ...
                      <td className="py-3 px-3">
                        <Input
                          ...
                          className="h-8 text-right font-mono text-sm bg-background"
                          min="0"
                        />
                      </td>
                      <td className="py-3 px-3">
                        <Select value={String(item.taxRate)} onValueChange={val => updateItem(item.id, 'taxRate', val)}>
                          <SelectTrigger className="h-8 text-center text-xs font-mono justify-center px-1 bg-background text-foreground">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            {GST_RATES.map(rate => (
                              <SelectItem key={rate.value} value={String(rate.value)}>{rate.value}%</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </td>
                      ...
                      <td className="py-3 px-3 text-center">
                        <Button
                          ...
                          className="size-7 text-muted-foreground hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                  ...
                  <tr className="hover:bg-muted/20 transition-colors">
                    ...
                    <td colSpan={7} className="py-3 px-3">
                      <Input
                        ...
                        className="h-8 border-transparent hover:border-border bg-transparent px-1 text-sm text-muted-foreground"
                        onFocus={addItem}
                        readOnly
                      />
                    </td>
            ...
            <div className="px-5 py-3 border-t border-border">
              <button
                type="button"
                onClick={addItem}
                className="flex items-center gap-1.5 text-sm font-semibold text-brand-primary hover:text-brand-primary-hover transition-colors"
              >
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  className="resize-none text-sm min-h-[90px] bg-card border-border"
                />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="bg-card border border-border rounded-xl p-5 space-y-3 text-sm self-start">
              ...
              <div className="pt-3 border-t border-border">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-2xl font-bold text-brand-primary font-mono leading-none">
      ...
      <div className="flex items-center justify-between px-6 py-3.5 bg-card border-t border-border">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={loading}
            className="h-9 px-5 text-sm font-medium gap-1.5 bg-brand-primary hover:bg-brand-primary-hover text-white shadow-sm"
          >
      ...
      {/* Dialogs */}
      <AddProductDialog open={showAddProduct} onOpenChange={setShowAddProduct} onSuccess={(p: any) => {
        setProducts(prev => [...prev, p]);
      }} />
      <AddCustomerDialog open={showAddCustomer} onOpenChange={setShowAddCustomer} onSuccess={(c: any) => {
