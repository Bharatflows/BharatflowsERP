...
export function CreateQuotation({ quotationId, defaultPreview = false, onSave, onCancel }: CreateQuotationProps) {
            ...
            try {
                ...
                if (quotationId) {
                    ...
                    if (quotation) {
                        ...
                        setQuotationDate(quotation.date ? new Date(quotation.date).toISOString().split("T")[0] : "");
                        setValidUntil(quotation.validUntil ? new Date(quotation.validUntil).toISOString().split("T")[0] : "");
                        setNotes(quotation.notes || "");
                        setTermsConditions(quotation.terms || "");
                    ...
                    // Fetch the next quotation number from the sequence API
                    try {
                        const response = await settingsService.getNextSequenceNumber('QUOTATION');
                        if (response.success && response.data?.nextNumber) {
                            setQuotationNumber(response.data.nextNumber);
                        }
                    } catch (error) {
    ...
    const handleSave = async () => {
        if (!customerId || !quotationDate || !validUntil || items.length === 0) {
        ...
        try {
            ...
            if (quotationId) {
                await salesService.updateQuotation(quotationId, quotationData as any);
                toast.success("Quotation updated successfully");
            } else {
                await salesService.createQuotation(quotationData as any);
                    ...
                    <CardContent className="p-8 md:p-12">
                        ...
                        <div className="flex justify-between items-start mb-12">
                            <div>
                                ...
                                <p className="text-muted-foreground">#{quotationNumber || "DRAFT"}</p>
                            </div>
                        ...
                        <div className="flex justify-between mb-12">
                            <div>
                                <p className="text-sm font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Quotation For</p>
                                {selectedCustomer ? (
                                    <div>
                            ...
                            <div className="text-right space-y-2">
                                <div>
                                    ...
                                    <p className="font-medium">{new Date(quotationDate).toLocaleDateString('en-IN', { dateStyle: 'long' })}</p>
                                </div>
                                <div>
                        ...
                        <div className="mb-8">
                            <div className="w-full">
                                <div className="border-b-2 border-primary/10 grid grid-cols-12 gap-4 py-3 font-semibold text-sm text-muted-foreground">
                                ...
                                <div className="divide-y divide-border">
                                    {items.map((item) => (
                                        <div key={item.id} className="grid grid-cols-12 gap-4 py-4">
                                            <div className="col-span-6">
                                                <p className="font-medium">{item.name}</p>
                                                {item.description && <p className="text-sm text-muted-foreground">{item.description}</p>}
                                            </div>
                                            <div className="col-span-2 text-center">{item.quantity}</div>
                                            <div className="col-span-2 text-right">₹{item.rate.toLocaleString('en-IN')}</div>
                                            <div className="col-span-2 text-right font-medium">₹{item.amount.toLocaleString('en-IN')}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
        ...
        <div className="max-w-6xl mx-auto p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-xl border border-border/50 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-foreground">{quotationId ? "Edit Quotation" : "New Quotation"}</h1>
                    ...
                    <Card className="border-border/50 shadow-sm overflow-hidden">
                        <div className="bg-muted/30 px-6 py-4 border-b border-border/50 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="font-semibold text-sm">Quotation Items</h3>
                            </div>
                ...
                <div className="space-y-6">
                    <Card className="border-border/50 shadow-sm">
                        <div className="bg-muted/30 px-6 py-4 border-b border-border/50 flex items-center gap-2">
                            ...
                            <h3 className="font-semibold text-sm">Quotation Settings</h3>
                        </div>
                        <CardContent className="p-6 space-y-4">
                            <div className="space-y-2">
                                <Label htmlFor="quotationNumber" className="text-xs font-medium uppercase text-muted-foreground">Quotation Number</Label>
                                <div className="flex items-center gap-2">
                            ...
                            <Separator />
                            <div className="space-y-2">
                                <Label htmlFor="quotationDate" className="text-xs font-medium uppercase text-muted-foreground">Quotation Date</Label>
                                <Input
                                    id="quotationDate"
                                    type="date"
                                    value={quotationDate}
