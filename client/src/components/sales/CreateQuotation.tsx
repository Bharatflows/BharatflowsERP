...
import {
    Trash2, Save, Send, ChevronLeft, Search, Loader2, Printer, PlusCircle, Eye, Download
} from "lucide-react";
...
export function CreateQuotation({ quotationId, defaultPreview = false, onSave, onCancel }: CreateQuotationProps) {
            ...
            try {
                ...
                if (quotationId) {
                    ...
                    try {
                        ...
                        if (response.success && response.data?.nextNumber) setQuotationNumber(response.data.nextNumber);
    ...
    const addItem = () => {
        setItems([...items, { id: Date.now().toString(), productId: undefined, name: "", description: "", hsn: "", quantity: 1, unit: "pcs", rate: 0, taxRate: 18, amount: 0 }]);
    };
        ...
        <div className="flex flex-col min-h-screen bg-background">
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-card border-b border-border">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-bold text-foreground">{quotationId ? "Edit Quotation" : "New Quotation"}</h1>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
                            <div className="space-y-1.5">
                                ...
                                <Input value={quotationNumber} onChange={e => setQuotationNumber(e.target.value)} placeholder="Auto-generated" className="h-9 font-mono text-sm w-full bg-muted/30 border-border" />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1.5">
                                    <Label className="text-xs font-medium text-muted-foreground">Quotation Date</Label>
                                    <Input type="date" value={quotationDate} onChange={e => setQuotationDate(e.target.value)} className="h-9 text-sm" />
                                </div>
                                <div className="space-y-1.5">
                                ...
                                <tbody className="divide-y divide-border/50">
                                    {items.map((item, index) => (
                                        <tr key={item.id} className="group hover:bg-muted/20 transition-colors">
                                            ...
                                            <td className="py-3 px-3"><Input value={item.hsn} onChange={e => updateItem(item.id, 'hsn', e.target.value)} placeholder="HSN" className="h-8 text-xs font-mono text-center bg-background" /></td>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Textarea value={termsConditions} onChange={e => setTermsConditions(e.target.value)} placeholder="Validity, payment terms..." className="resize-none text-sm min-h-[90px] bg-card border-border" />
                            </div>
                        </div>
            ...
            <div className="flex items-center justify-between px-6 py-3.5 bg-card border-t border-border">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button onClick={() => handleSave(true)} disabled={loading} className="h-9 px-5 text-sm font-medium gap-1.5 bg-brand-primary hover:bg-brand-primary-hover text-white shadow-sm">
                        {loading ? <Loader2 className="animate-spin size-4" /> : <Send className="size-4" />} Send Quotation
                    </Button>
                </div>
            </div>
