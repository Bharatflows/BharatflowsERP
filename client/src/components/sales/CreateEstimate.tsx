...
import {
  Plus, Trash2, Save, Send, ChevronLeft, Search, Loader2, Printer, PlusCircle, ArrowRightCircle
} from "lucide-react";
...
import { GST_RATES, calculateTax } from "../../constants/tax";
...
export function CreateEstimate({ estimateId, defaultPreview = false, onSave, onCancel }: CreateEstimateProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (estimateId) {
          const response: any = await salesService.getEstimate(estimateId);
          ...
          if (estimate) {
            ...
            setEstimateDate(estimate.date ? new Date(estimate.date).toISOString().split("T")[0] : "");
            setValidUntil(estimate.validUntil ? new Date(estimate.validUntil).toISOString().split("T")[0] : "");
          ...
          try {
            const response = await settingsService.getNextSequenceNumber('ESTIMATE');
            if (response.success && response.data?.nextNumber) {
              setEstimateNumber(response.data.nextNumber);
            }
          } catch (error) {
            console.error("Failed to fetch next estimate number:", error);
          }
        }
      } catch (error) {
  ...
  const addItem = () => {
    setItems([...items, {
      id: Date.now().toString(), name: "", description: "", hsn: "",
      quantity: 1, unit: "pcs", rate: 0, taxRate: 18, amount: 0
    }]);
  };
  ...
  const taxBreakdown = (() => {
    ...
    items.forEach(item => {
      ...
      const t = calculateTax(line, String(item.taxRate), false);
      cgst += t.cgst; sgst += t.sgst;
    });
    return { cgst, sgst };
  })();
  const totalTax = taxBreakdown.cgst + taxBreakdown.sgst;
  const total = subtotal + totalTax - discount;
  const gstRate = items.length > 0 ? items[0].taxRate : 0;
  ...
  const handleConvert = async () => {
    ...
    try {
      if (window.confirm("Convert this estimate to an invoice?")) {
        const response: any = await salesService.convertEstimateToInvoice(estimateId);
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-card border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-foreground">{estimateId ? "Edit Estimate" : "New Estimate"}</h1>
          <Badge variant="outline" className="text-xs font-medium bg-muted/50 text-muted-foreground border-border">{status}</Badge>
        </div>
        <div className="flex items-center gap-2">
          {estimateId && status !== 'converted' && (
            <Button variant="outline" onClick={handleConvert} className="h-9 px-4 text-sm font-medium gap-1.5 text-blue-600 border-blue-200 hover:bg-blue-50">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-card border border-border rounded-xl p-5 space-y-4">
              <div className="space-y-1.5">
                <Label className="text-xs font-medium text-muted-foreground">Estimate Number</Label>
                <Input value={estimateNumber} onChange={e => setEstimateNumber(e.target.value)} placeholder="Auto-generated" className="h-9 font-mono text-sm w-full bg-muted/30 border-border" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground">Estimate Date</Label>
                  <Input type="date" value={estimateDate} onChange={e => setEstimateDate(e.target.value)} className="h-9 text-sm" />
                </div>
                <div className="space-y-1.5">
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            ...
            <div className="px-5 py-3 border-t border-border">
              <button type="button" onClick={addItem} className="flex items-center gap-1.5 text-sm font-semibold text-brand-primary hover:text-brand-primary-hover transition-colors">
                <PlusCircle className="size-4" /> Add Another Line
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea value={termsConditions} onChange={e => setTermsConditions(e.target.value)} placeholder="Validity period, payment terms..." className="resize-none text-sm min-h-[90px] bg-card border-border" />
              </div>
            </div>
              ...
              <div className="pt-3 border-t border-border">
                <div className="flex justify-between items-end">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5 max-w-[220px] leading-snug">In Words: {numberToWords(total)}</p>
                  </div>
