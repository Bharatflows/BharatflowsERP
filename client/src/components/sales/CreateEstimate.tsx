...
import { useCreateEstimate, useUpdateEstimate } from "../../hooks/useSales";
...
export function CreateEstimate({ estimateId, defaultPreview = false, onSave, onCancel }: CreateEstimateProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // If editing, fetch estimate details
        if (estimateId) {
          const estimate = await salesService.getEstimate(estimateId);
          if (estimate) {
          ...
          // Fetch the next estimate number from the sequence API
          try {
  ...
  const handleSave = async () => {
    if (!customerId || !estimateDate || !validUntil || items.length === 0) {
    ...
    try {
      if (estimateId) {
        await updateEstimateMutation.mutateAsync({ id: estimateId, data: estimateData });
      } else {
        ...
        <Card className="border-0 shadow-lg bg-white overflow-hidden">
          <CardContent className="p-8 md:p-12">
            ...
            <div className="flex justify-between items-start mb-12">
              <div>
                <h1 className="text-4xl font-bold text-primary mb-2">ESTIMATE</h1>
                <p className="text-muted-foreground">#{estimateNumber || "DRAFT"}</p>
              </div>
            ...
            <div className="flex justify-between mb-12">
              <div>
                <p className="text-sm font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Estimate For</p>
                {selectedCustomer ? (
                  <div>
              ...
              <div className="text-right space-y-2">
                <div>
                  <p className="text-sm font-semibold text-muted-foreground">Estimate Date</p>
                  <p className="font-medium">{new Date(estimateDate).toLocaleDateString('en-IN', { dateStyle: 'long' })}</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-muted-foreground">Valid Until</p>
                  <p className="font-medium">{validUntil ? new Date(validUntil).toLocaleDateString('en-IN', { dateStyle: 'long' }) : '-'}</p>
                </div>
              </div>
            </div>
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-xl border border-border/50 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">{estimateId ? "Edit Estimate" : "New Estimate"}</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={handleSave} disabled={isSaving} className="gap-2 bg-primary text-primary-foreground shadow-sm hover:shadow-md transition-all">
        ...
        <div className="space-y-6">
          <Card className="border-border/50 shadow-sm">
            <div className="bg-muted/30 px-6 py-4 border-b border-border/50 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-sm">Estimate Settings</h3>
            </div>
            <CardContent className="p-6 space-y-4">
              <div className="space-y-2">
                <Label htmlFor="estimateNumber" className="text-xs font-medium uppercase text-muted-foreground">Estimate Number</Label>
                <div className="flex items-center gap-2">
              ...
              <Separator />
              <div className="space-y-2">
                <Label htmlFor="estimateDate" className="text-xs font-medium uppercase text-muted-foreground">Estimate Date</Label>
                <Input
                  id="estimateDate"
                  type="date"
                  value={estimateDate}
              ...
              <div className="space-y-2">
                <Label htmlFor="validUntil" className="text-xs font-medium uppercase text-muted-foreground">Valid Until</Label>
                <Input
                  id="validUntil"
                  type="date"
                  value={validUntil}
